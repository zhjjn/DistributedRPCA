...
for random_num in range(2010,2012):
    ...
    print X_train.shape
    dis_rf_acc, dis_svm_acc, dis_lr_acc, dis_rf_time, dis_svm_time, dis_lr_time = split(X_train, y_train, 4, X_train.shape[0], None)
    ...
    print "Centralized RPCA+Random Forest Accuracy: ", acc
    t5 = time.time()
    central_rf_t = t5 - t4 + t4 - t3
    central_rf_acc.append(acc)
    central_rf_time.append(central_rf_t)
    ...
    acc = counts * 1.0 / len(y_preds)
    print "Centralized RPCA+SVM Accuracy: ", acc
    t7 = time.time()
    central_svm_t = t7 - t6 + t4 - t3
    central_svm_acc.append(acc)
    central_svm_time.append(central_svm_t)

    t8 = time.time()
    lr_model = LogisticRegression()
    lr_model.fit(X_train_PCA, y_train)

    acc = lr_model.score(X_train_PCA, y_train)
    print "Centralized RPCA+LogicRegression Accuracy: ", acc
    t9 = time.time()
    central_lr_t = t9 - t8 + t4 - t3
    central_lr_acc.append(acc)
    central_lr_time.append(central_lr_t)
...
central_rf_acc_average = sum(central_rf_acc) / len(central_rf_acc)
print "Average RPCA+RandomForest Accuracy: ", central_rf_acc_average
central_rf_time_average = sum(central_rf_time) / len(central_rf_time)
print "Average RPCA+RandomForest Time: ", central_rf_time_average

central_svm_acc_average = sum(central_svm_acc) / len(central_svm_acc)
print "Average RPCA+SVM Accuracy: ", central_svm_acc_average
central_svm_time_average = sum(central_svm_time) / len(central_svm_time)
print "Average RPCA+SVM Time: ", central_svm_time_average

central_lr_acc_average = sum(central_lr_acc) / len(central_lr_acc)
print "Average RPCA+LogicRegression Accuracy: ", central_lr_acc_average
central_lr_time_average = sum(central_lr_time) / len(central_lr_time)
print "Average RPCA+LogicRegression Time: ", central_lr_time_average

f = open("Central&4nodes_RPCA+Classifier.txt", 'a')
f.write("Average RPCA+RandomForest Accuracy: "+str(central_rf_acc_average)+"\n")
f.write("Average RPCA+RandomForest Time: "+str(central_rf_time_average)+"\n")
f.write("Average RPCA+SVM Accuracy: "+str(central_svm_acc_average)+"\n")
f.write("Average RPCA+SVM Time: "+str(central_svm_time_average)+"\n")
f.write("Average RPCA+LogicRegression Accuracy: "+str(central_lr_acc_average)+"\n")
f.write("Average RPCA+LogicRegression Time: "+str(central_lr_time_average)+"\n")
